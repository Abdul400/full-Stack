...
let myDailyLink = document.getElementById('daily'), myWeeklyLink = document.getElementById('weekly'), myMonthlyLink = document.getElementById('monthly'), when = document.querySelectorAll('.when'), duration = document.querySelectorAll('.duration'), time = document.querySelectorAll('.time');
console.log(time);
console.log(duration);
//functions for changing data
//daily data
function changeMyDailyData() {
    for(let i = 0; i < time.length; i++)for(let i1 = 0; i1 < myData.length; i1++)time[i1].innerHTML = myData[i1].timeframes.daily.current + 'hrs';
    for(let i2 = 0; i2 < duration.length; i2++)for(let i3 = 0; i3 < myData.length; i3++)duration[i3].innerHTML = myData[i3].timeframes.daily.previous + 'hrs';
}
//weekly data
function changeMyWeeklyData() {
    for(let i = 0; i < time.length; i++)for(let i4 = 0; i4 < myData.length; i4++)time[i4].innerHTML = myData[i4].timeframes.weekly.current + 'hrs';
    for(let i5 = 0; i5 < duration.length; i5++)for(let i6 = 0; i6 < myData.length; i6++)duration[i6].innerHTML = myData[i6].timeframes.weekly.previous + 'hrs';
}
//Montly data
function changeMymonthlyData() {
    for(let i = 0; i < time.length; i++)for(let i7 = 0; i7 < myData.length; i7++)time[i7].innerHTML = myData[i7].timeframes.monthly.current + 'hrs';
    for(let i8 = 0; i8 < duration.length; i8++)for(let i9 = 0; i9 < myData.length; i9++)duration[i9].innerHTML = myData[i9].timeframes.monthly.previous + 'hrs';
}
//default data view when page loads
window.onload = ()=>{
    changeMyDailyData();
    for(let i = 0; i < when.length; i++)when[i].textContent = 'Yesterday -';
};
//Eventlisteners
//adding eventlistener to the daily button
myDailyLink.addEventListener('click', ()=>{
    changeMyDailyData();
    for(let i = 0; i < when.length; i++)when[i].textContent = 'Yesterday -';
});
//adding eventlistener to the weekly button
myWeeklyLink.addEventListener('click', ()=>{
    changeMyWeeklyData();
    for(let i = 0; i < when.length; i++)when[i].textContent = 'Last Week -';
});
//adding eventlistener to the montly button
myMonthlyLink.addEventListener('click', ()=>{
    changeMymonthlyData();
    for(let i = 0; i < when.length; i++)when[i].textContent = 'Last Month -';
    for(let i10 = 0; i10 < duration.length; i10++)duration[i10].style.marginLeft = '3px';
});

},{"bootstrap":"h36JB","../data.json":"cn6Iz"}],"h36JB":[function(require,module,exports) {
...
},{"./createPopper.js":"cHuNp","./modifiers/eventListeners.js":"hBKsL","./modifiers/popperOffsets.js":"6I679","./modifiers/computeStyles.js":"gDlm2","./modifiers/applyStyles.js":"4iMn4","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"cn6Iz":[function(require,module,exports) {
...
},{}]},["7nZVA","8lqZg"], "8lqZg", "parcelRequiref3e3")
